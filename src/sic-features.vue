      ...
      <v-flex xs12 v-for="(p,i) in features" :key="p.id">
        <v-card color="indigo lighten-2" class="white--text">
          <v-toolbar flat color="indigo lighten-1" dark height="20px">
            ...
            <v-btn v-if="editable" flat dark icon small style="margin-left: -5px;" @click.native.stop="viewCard(i)">
              <v-icon v-if="p.backLogItemId" size="14px">fa-briefcase</v-icon>
            </v-btn>
            <v-btn v-if="enableComments" flat dark icon small style="margin-left: 0px;" @click.native.stop="$emit('newFeatureComment', p)">

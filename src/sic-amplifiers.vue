<template>
  <v-card color="teal lighten-3">
    <v-toolbar flat color="teal lighten-2" dark height="30px">
      ...
      <v-toolbar-title style="margin-left: 0px;" class="body-1">Amplifiers</v-toolbar-title>
    </v-toolbar>
    <v-container fluid style="min-height: 0;" grid-list-lg>
      <v-layout row wrap>
        <v-flex xs12 v-for="(p,i) in amplifiers" :key="p.id">
    ...
    <v-dialog v-model="dialog" persistent max-width="500px">
      <v-card>
        <v-card-title>
          <span class="headline">Amplifier</span>
        </v-card-title>
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12 sm12 md8>
                <v-text-field label="Name" required v-model="amplifier.name"></v-text-field>
              </v-flex>
              <v-flex xs12 sm6>
                <v-select label="Type" multiple autocomplete chips v-model="amplifier.type"
                  :items="['Service', 'Product']">
                </v-select>
              </v-flex>
              <v-flex xs12 sm12 md12>
                <v-text-field multi-line label="Description" v-model="amplifier.description"></v-text-field>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...
    edit: function(i) {
      this.amplifier =  Object.assign( {}, this.amplifiers[i]);
      this.editIdx = i;
      this.dialog=true;
    },
    save: function(i) {
      ...
      if (!this.amplifier.id) {
        this.amplifier.id=uuidv1();
        this.amplifiers.push(this.amplifier);
      } else {
        this.amplifiers[this.editIdx] = Object.assign( {}, this.amplifier);
      }
    }
  }
}
